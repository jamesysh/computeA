...
__global__ void computeA3D_gpu(const int*neighbourList, int numRow, const double*x, const double*y,
const double*z, int startIndex, int numComputing, int maxNeighbourOneDir,
        double**A,double*dis)//output
   {
    ...
    while(tid < numComputing){
        ...
        int numOfRow = numRow;
        double distance = sqrt((x[neighbourList[index*maxNeighbourOneDir]]-x[index])*(x[neighbourList[index*maxNeighbourOneDir]]-x[index])+(y[neighbourList[index*maxNeighbourOneDir]]-y[index])*(y[neighbourList[index*maxNeighbourOneDir]]-y[index])+(z[neighbourList[index*maxNeighbourOneDir]]-z[index])*(z[neighbourList[index*maxNeighbourOneDir]]-z[index]));
        if(LPFOrder[index] == 1){
