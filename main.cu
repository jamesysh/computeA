...
//--------------------gpu------------------------------------
    ...
    computeA3D_gpu<<<96,32>>>(d_neighbourlist0 ,d_LPFOrder0 ,numRow ,d_xPosition ,d_yPosition ,d_zPosition ,0 ,numFluid ,numNeighbourInOne ,
            A ,d_distance);
    ...
    printf("GPU time of computeA is %f(ms)\n",gputime);
    }
...
//--------------cpu------------------------------------
    ...
    omp_set_num_threads(min(omp_get_max_threads(),1));    
    ...
    cout<<"CPU time of computeA is "<<time_cpu*1000<<"(ms)"<<endl;  

}
    
   /*
for(int i=33485;i<33486;i++){
